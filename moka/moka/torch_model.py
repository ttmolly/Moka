"""Export-only ModernBERT graph, adapted from laya-mlx / laya-coreml; see NOTICE.
...
The default ONNX lowering uses the explicit matmul/softmax path: SDPA often
emits FlashAttention-style ops that CPU Execution Provider cannot run.
"""
...
def init_untrained(model: DecisionModel):
    """Xavier-init the empty in_proj parameters used by the decision head."""
